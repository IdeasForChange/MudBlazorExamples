...
namespace EnterpriseAppFluxor.Features.Theme.Store;
...
public class ThemeEffects
{
    ...
    [EffectMethod]
    public async Task OnLoadDarkMode(ThemeLoadDarkModeAction action, IDispatcher dispatcher)
    {
        //var appSetting = await _appSettingRepo.GetOrAdd(APP_SETTING_IS_DARK_MODE, "false");
        dispatcher.Dispatch(new ThemeSetDarkModeAction(false));
    }

    [EffectMethod]
    public async Task OnToggleDarkModeAction(ThemeToggleDarkModeAction action, IDispatcher dispatcher)
    {
        //var appSetting = await _appSettingRepo.Toggle(APP_SETTING_IS_DARK_MODE);
        dispatcher.Dispatch(new ThemeSetDarkModeAction(true));
    }
}
